 ...
 * and routing http requests for the customer route
 */
@RestController
public class CustomerController {
    ...
    /**
     * function that handle Post requests for creating new  customer
     *
     ...
     *
     * @return Collection model of entities model of Dto's customers with status code "ok"
     */
    ...
    ResponseEntity<?> updatedCustomer(@RequestBody Customer newCustomer,
                ...
                .map(updated -> {
                    try {
                        return ResponseEntity.created(new URI(
                                updated.getRequiredLink(IanaLinkRelations.SELF).getHref())).body(updated);
                    } catch (URISyntaxException e) {
                        e.printStackTrace();
                    }
                    return ResponseEntity.internalServerError().body("Something went wrong while updating customer" + id);
                })
                .orElseThrow(() -> new CustomerNotFoundException(id));
    }
}
